...
export function StoryCard({ 
  ...
  const [chatMessages, setChatMessages] = useState<Array<{type: 'user' | 'agent', content: string, character?: string, isOpening?: boolean}>>(() => {
    console.log('[Debug] 初始化消息 - initialMessages:', initialMessages);
    if (initialMessages.length > 0) {
      const lastUserIndex = initialMessages.findLastIndex(msg => msg.type === 'user');
      console.log('[Debug] 初始化消息 - 最后用户消息索引:', lastUserIndex);
      const filteredMessages = lastUserIndex !== -1 
        ? initialMessages.slice(lastUserIndex)
        : initialMessages.slice(-1);
      console.log('[Debug] 初始化消息 - 筛选后:', filteredMessages);
  ...
  const addMessage = (type: 'user' | 'agent', content: string, character?: string, character_id?: string) => {
    console.log('[Debug] 添加消息:', { type, content, character, character_id });
    setChatMessages(prevMessages => {
      console.log('[Debug] 当前消息列表:', prevMessages);
      if (type === 'agent') {
        // 如果是AI回复，保留最后一条用户消息和新的回复
        ...
        // 如果是用户消息，开始新的对话
        const newMessages = [{ type, content }];
        console.log('[Debug] 更新后的消息列表:', newMessages);
        return newMessages;
      }
    });
  ...
  const handleUserInput = async (content: string) => {
    try {
      ...
      let currentHistoryMessages = await getHistoryMessages(conversationId!, lastMessageTime)
      ...
      for (let i = 0; i < speakers.length; i++) {
        ...
        if (i > 0) {
          const futureTime = new Date(Date.now() + 2000)
          const lastMessageTime = futureTime.toISOString()
          currentHistoryMessages = await getHistoryMessages(conversationId!, lastMessageTime)
        }
  ...
  const fetchConversationHistory = async (storyId: string) => {
    try {
      ...
      if (existingConversationId) {
        ...
        console.log('[Debug] 原始历史消息:', historyMessages);
        ...
        if (historyMessages && historyMessages.length > 0) {
          ...
          console.log('[Debug] 格式化后的消息:', formattedMessages);

          // 找到最后一个用户消息的索引
          const lastUserIndex = formattedMessages.findLastIndex(msg => msg.type === 'user');
          console.log('[Debug] 最后一个用户消息索引:', lastUserIndex);
          
          // 只保留从最后一个用户消息开始的对话
          const filteredMessages = lastUserIndex !== -1 
            ? formattedMessages.slice(lastUserIndex)
            : formattedMessages.slice(-1);
            
          console.log('[Debug] 最终筛选后的消息:', filteredMessages);
          
          // 更新消息列表
          setChatMessages(filteredMessages);
          
          // 通知父组件新消息
          onNewMessages(filteredMessages);

          // 如果有历史消息，保存最后一条消息的ID
          if (historyMessages.length > 0) {
            setLastMessageId(historyMessages[historyMessages.length - 1].id);
          }
        }
      } else {
        console.warn('No existing conversation found for story:', storyId);
      }
    } catch (error) {
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Avatar className="w-8 h-8 border border-white/20">
                  <AvatarImage src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/sleek-cat-paw-icon-2-7Csmt4QTEimQiNe5rtjKP8ltLSmy7w.png" />
